...
$json_output = json_decode(get_url_content($jsonurl));
...
for($i=0; $i<$num_results; $i++){
  ...
  $borrower_geo_json =  json_decode(get_url_content("https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($country_codes[$country_code])."&sensor=false"));
  ...
  foreach($supplier_countries as $country){
    $geo_json_url = get_url_content("https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($country)."&sensor=false");
